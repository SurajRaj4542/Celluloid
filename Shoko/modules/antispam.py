...
Shoko_gban.create_api_token('Erina gban', 'Erina', 'https://www.meanii.me/') 
...
def gban(update, context):
    ...
    page = Shoko_gban.post(title=f"Erina-gban-{user_chat.id}", author=f"{banner.first_name} ({banner.id})", text=EVIDENSE_NEW_GBAN)
    evidence_link = page.get('url')
    evidence_link = "<a href='{}'>{}</a>".format(evidence_link, f"Erina GBanned // user_id: {user_chat.id}")
